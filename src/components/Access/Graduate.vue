<template>
  ...
  >
      ...
      <div class="flex flex-wrap items-center">
        <div class="relative w-full px-4 max-w-full flex-grow flex-1">
          <h6 class="uppercase text-blueGray-400 mb-1 text-xs">
            Facultad de Ingeniería y Arquitectura y Facultad de Ciencia y
            Tecnología
          </h6>
          <h2 class="text-blueGray-700 text-xl">Graduados por carrera</h2>
        </div>
      </div>
    </div>
    <!-- Filtros -->
    ...
    <!-- Gráfico -->
    <div class="w-full px-4 chartContainer">
...
export default {
  ...
  computed: {
    ...
    filteredData() {
      return this.enrollmentData.filter(
        (item) =>
          this.selectedCarrera === "all" ||
          item.carrera === this.selectedCarrera
      );
    },
    chartData() {
      const years =
        this.selectedAno === "all" ? ["2023", "2024"] : [this.selectedAno];
      const genders =
        this.selectedGenero === "all"
          ? ["masculino", "femenino"].reverse()
          : [this.selectedGenero];
        ...
        datasets: genders.map((gender) => ({
          ...
          data: this.filteredData.flatMap((carrera, index) =>
            years.map((year) => ({
              x: year === "2023" ? 25 : 75,
              y: index,
              r: Math.sqrt(carrera[year][gender].count) * 5, 
              carrera: carrera.carrera,
              year: year,
              count: carrera[year][gender].count,
            }))
          ),
          borderColor: gender === "masculino" ? "#41D3BF" : "#41D3BF",
          backgroundColor:
            gender === "masculino" ? "#fff" : "#41D3BF",
          borderWidth: 2,
        })),
      };
    },
    ...
    renderChart() {
      ...
      this.chartInstance = new Chart(ctx, {
        ...
        options: {
          ...
          scales: {
            x: {
              // Ahora el eje X muestra los años
              min: 0,
              max: 100,
              ticks: {
                callback: (value) => {
            ...
            y: {
              // Ahora el eje Y muestra las carreras
              min: -1,
